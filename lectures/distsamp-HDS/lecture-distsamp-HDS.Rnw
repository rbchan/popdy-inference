...
\begin{frame}[fragile]
  ...
  \footnotesize
<<un-fit,size='tiny'>>=
fm <- distsamp(~noise ~elevation, umf)    
fm
@
\pause
\vfill
...
\begin{frame}[fragile]
  ...
  \footnotesize
<<un-fit-pt,size='tiny'>>=
fm4 <- distsamp(~noise ~elevation, umf4)    
fm4
@
\pause
\vfill
...
\begin{frame}[fragile]
  \frametitle{MCMC}
  \small
<<bugs-mcmc-pt,size='scriptsize',message=FALSE,cache=FALSE,results='hide'>>=
