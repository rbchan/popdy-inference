...
\begin{frame}
  ...
  Detection probability is treated as a function of distance ($x$). \\
  \pause
  \vfill
  \centering
...
\begin{frame}
  ...
  Average detection probability is defined as:
  \[
     \bar{p} = \int g(x)p(x) \; \mathrm{d}x
  \]
  \pause
  \vfill
  To compute $\bar{p}$, we have to specify $p(x)$, the
