...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{sigma} \hlkwb{<-} \hlnum{25}
\hlstd{(pbar} \hlkwb{<-} \hlstd{(sigma}\hlopt{^}\hlnum{2}\hlopt{*}\hlstd{(}\hlnum{1}\hlopt{-}\hlkwd{exp}\hlstd{(}\hlopt{-}\hlstd{B}\hlopt{^}\hlnum{2}\hlopt{/}\hlstd{(}\hlnum{2}\hlopt{*}\hlstd{sigma}\hlopt{^}\hlnum{2}\hlstd{)))} \hlopt{-}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Repeat step 1, but for point-transect sampling. 
  \end{enumerate}
\end{frame}
