...
simulation, fitting, and prediction %, and random effects
...
If we can estimate the function describing how $p$ declines with distance $x$, we can estimate abundance\dots if certain assumptions hold, as always.
...
In distance sampling, detection probability is a \alert{function} of distance, rather than a constant, such that all individuals have unique detection probabilities.
  
  
As a result, we have to replace $p$ with \alert{average} detection probability:
$$
...
The most common detection functions are: <br>
...
- Here, $x$ is the \alert{radial} distance to an animal
  

## Computing average *p* for line transects
...
## Computing average *p* for point transects
...
##  Conventional vs hierarchical distance sampling
...
%    \pi(b_1, \dots, b_{J+1}, x, \sigma_i))
  ...
  $w_1$, $w_2$, $w_3$ -- site covariates % %<br>
...
## Multinomial cell probs for point transects
  ...
  multinomial cell probabilities. 
    ...
    * $b_1, \dots, b_J$ -- Distance break points defining $J$
      distance intervals
    % * $a_1, \dots, a_J$ -- Area of annulus $j$
    % * $\bar{p}_j = \int_{b_j}^{b_{j+1}} g(x,\sigma)p(x|b_j\le x<b_{j+1})\, \mathrm{d}x$
    % * $p(x|b_j\le x<b_{j+1}) = 2\pi x/a_j$
    % * $\psi_j=\Pr(b_j\le x<b_{j+1})=a_j/(\pi B^2)$
...
## Multinomial cell probs for line transects
...
Definitions needed for computing \alert{bin-specific} $\bar{p}$ and multinomial cell probabilities. 
    ...
    * $b_1, \dots, b_{J+1}$ -- Distance break points defining $J$ distance intervals
    % * $\bar{p}_j = \int_{b_j}^{b_{j+1}} g(x,\sigma)p(x|b_j\le x<b_{j+1})\, \mathrm{d}x$
    % * $p(x|b_j\le x<b_{j+1}) = 1/(b_{j+1}-b_j)$
    * $\bar{p}_j$ -- Average detection probability in distance bin j
    % * $\psi_j=\Pr(b_j\le x<b_{j+1})=(b_{j+1}-b_j)/B$ % -- Pr(occuring in distance bin $j$)
...
* Density covariates: {\tt Elevation, UTM.N, UTM.W}
* Detection covariates: {\tt Wind, Noise}
* Response: {\scriptsize \tt btbw0\_20, btbw20\_40, btbw40\_60, btbw60\_80, btbw80\_100}
