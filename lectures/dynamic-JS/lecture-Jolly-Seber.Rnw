...
caps <- data.frame(session=1,
                   animal=rep(slice.index(y, 1), y.sp.secr),
                   occasion=rep(slice.index(y, 3), y.sp.secr),
                   trap=rep(slice.index(y, 2), y.sp.secr))
...
##                   start=start)
                   start=list(lambda0=0.3, sigma=100, D=0.1, phi=0.5, beta=0.2))
mod$fit()
mod
@   
Back-transform
...
par(mai=c(0.9,0.9,0.1,0.1))  
plot(1:T, N, type="b", ylim=c(0, 100), xlab="Time", ylab="Abundance", pch=16)
...
legend(1, 100, c("Actual abundance", "Estimated abundance"),
       col=c("black", gray(0.7)), lty=c(1,1), pch=c(16,16))
@
\end{frame}
