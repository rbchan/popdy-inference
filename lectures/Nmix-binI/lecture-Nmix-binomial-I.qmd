...
%%   %   
%%   
%%   Graph $p$ as a function of temperature. Include 95\% confidence intervals. <br>
%% 
...
\subsection{Bayesian methods: prior prediction}
  ...
  \alert{Prior predictive checks} can help us answer these questions. <br>
...
```
  ...
  [width=0.9\textwidth]{figure/prior-post-pred-2-1}  <br>
  ...
  There are two common strategies for specifying priors:
    ...
    * Use uniform/flat priors to obtain results that will often be
...
%    
    ...
    %% * Fit a binomial $N$-mixture model to the Canada warbler data
    %%   using `unmarked' and JAGS. The data include: 
    %%   
    %%     \footnotesize
    %%     * Response: \texttt{cawa1, cawa2, cawa3, cawa4}
    %%     * Site covs: \texttt{Elevation, Wind, Noise}
    %%   
    %% * Graph the predictions of $\lambda$ over the 
