...
\begin{frame}[fragile]
  ...
  \small
  Push the priors through the model to predict $p$
...
\begin{frame}
  ...
  If you don't have strong prior knowledge, it's a good idea to use
  priors that reflect plausible values of $\lambda$ and $p$. \\
  \pause
  \vfill
  We want to allow for the possibility of being ``surprised'' by the
  data, but we don't want to put most of the prior weight on extreme
  values. \\
  \pause
  \vfill
  ...
  \pause
  \vfill
  Prior predictive checks should be a standard component of Bayesian
  analysis. \\
\end{frame}
...
%    \small
    ...
    \item Using the simulated data, compare the prior and posterior
      ...
      types. Write 1-2 sentences summarizing the differences.
    \item Change the priors for $\alpha_0$ and $\alpha_1$ from
      ...
      sensitivie is the posterior to the prior?
    \item Fit a binomial $N$-mixture model to the Canada warbler data
      ...
      \end{itemize}
    \item Graph the prediction of $\lambda$ as a function of
      elevation, along with 95\% CIs.
  \end{enumerate}
  \vfill
