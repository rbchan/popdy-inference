...
Occupancy models: covariates and prediction} <br>
...
- [Observation covariates]{style="color: Bittersweet;"} may vary among
      sites and occasions
  
In both cases, covariates can be continuous or categorical. <br>
  
  
Observation covariates cannot be used to describe variation in occupancy because occupancy is assumed to be <br> constant over time. <br> 
...
\begin{gather*}
    ...
    + \alpha_2 {\color{Bittersweet} w_{ij}} + \cdots <br>
    y_{ij} \sim \mathrm{Bern}(z_i\times p_{ij})
  \end{gather*}
$$

  $\color{blue} x_1$ and $\color{blue} x_2$ are site covariates. <br>
   Examples: elevation, forest cover, distance to water. <br>
  
  
  $\color{Bittersweet} w$ is an observation covariate. <br>
...
```{r, un-umf,size='tiny',warning=FALSE}
...
It's almost always a good idea to standardize \alert{continuous} covariates. <br>
  
Standardizing involves subtracting the mean and then dividing by the standard deviation. <br>
  

```{r, umf-zcov,size='footnotesize'}
...
Create a data frame with prediction covariates. We'll let elevation
  vary while holding the other two covariates constant. <br>
...
```{r, pred-psi1,fig.width=7,fig.height=5,out.width='80%',fig.align='center',echo=-1}
par(mai=c(0.9,0.9,0.1,0.1))  
...
```{r, pred-psi1s,fig.width=7,fig.height=5,out.width='80%',fig.align='center',echo=-1}
par(mai=c(0.9,0.9,0.1,0.1))  
...
```{r, bugs,echo=FALSE,background='beige',comment=''}
...
```{r, bugs-mcmc,message=FALSE,cache=FALSE}
library(jagsUI)
...
```{r, bugs-plot1,out.width="70%",fig.align='center'}
...
```{r, bugs-plot2,out.width="70%",fig.align='center'}
...
Every MCMC iteration represents a sample from the posterior distribution. <br>
  
  
Each sample of the occupancy and detection coefficients can be used to make a prediction. <br>
  
  
The collection of predictions can be used to summarize the posterior  predictive distribution. <br>
  
We will look at the distribution of prediction lines. <br>  
...
```{r, psi-pred1,fig.align='center',out.width='80%',fig.height=5,dev='png',dpi=200}
...
```{r, psi-pred-post,fig.align='center',out.width='80%',fig.height=5,echo=-1,dev='png',cache=FALSE,dpi=200}
...
```{r, psi-pred-post-meanCI,fig.align='center',out.width='80%',fig.height=5,echo=-(1:2),dev='png',cache=FALSE,dpi=200}
plot(pred.data$elev, psi.post.pred[1,], type="l", xlab="elev",
     ylab="Occurrence probability", ylim=c(0, 1), col=gray(0.8))
for(i in 1:n.iter) {
    lines(pred.data$elev, psi.post.pred[i,], col=gray(0.8))
...
* The definition of a `site' is critical, but can difficult in
   some situations, such as in camera studies.
* Abundance is assumed to be unimportant
* Observations are assumed to be independent, conditional
...
* Dynamic occupancy models
      ...
      * Occupancy evolves as a function of local colonization
        and extinction
...
1. Fit an occupancy model in `unmarked' to the Ruffed Grouse
      data (`"grouse_data.csv"`). Use all three covariates, with
...
2. Make one prediction graph for occupancy and one for
      detection.
