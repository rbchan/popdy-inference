...
lines(lower ~ x1, psi.pred, col="grey")
lines(upper ~ x1, psi.pred, col="grey")
@   
\end{frame}
...
plot(Predicted ~ x1, p.pred, type="l", ylab="Detection probability", col="purple")
lines(lower ~ x1, p.pred, col="grey")
lines(upper ~ x1, p.pred, col="grey")
@   
\end{frame}
...
plot(Predicted ~ x1, psi.pred, type="l", ylab="Probability", col="blue")
lines(Predicted ~ x1, p.pred, col="purple")
...
jags.data <- list(y=y, x1=x1, x2=x2, wHot=wHot,
...
for(i in 1:n.iter) {
    psi.post.pred[i,] <- plogis(psi.coef.post[i,"beta0"] +
                                psi.coef.post[i,"beta1"]*pred.data$x1)
}
@ 
\end{frame}
...
<<psi-pred1,size='scriptsize',fig.align='center',out.width='80%',fig.height=5,dev='png'>>=
...
<<psi-pred-post,size='scriptsize',fig.align='center',out.width='80%',fig.height=5,echo=-1,dev='png',cache=TRUE>>=
plot(pred.data$x1, psi.post.pred[1,], type="l", xlab="x1",
     ylab="Occurrence probability", ylim=c(0, 1), col=gray(0.8))
for(i in 1:n.iter) {
...
<<psi-pred-post-meanCI,size='tiny',fig.align='center',out.width='80%',fig.height=5,echo=-(1:2),dev='png',cache=TRUE>>=
plot(pred.data$x1, psi.post.pred[1,], type="l", xlab="x1",
     ylab="Occurrence probability", ylim=c(0, 1), col=gray(0.8))
for(i in 1:n.iter) {
    lines(pred.data$x1, psi.post.pred[i,], col=gray(0.8))
