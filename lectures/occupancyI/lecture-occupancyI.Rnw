...
<<bup>>=
z.post <- ranef(fm)  
z.post.mean <-  bup(z.post, stat="mean")
sites.occupied.post <- predict(z.post, func=function(x) sum(x), nsim=1000)
plot(table(sites.occupied.post))
abline(v=sum(z), col="red")
@   
\end{frame}
