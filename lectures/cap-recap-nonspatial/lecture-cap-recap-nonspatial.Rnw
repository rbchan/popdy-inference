...
<<RMark-Mh,size='scriptsize',warning=FALSE,results='hide',cache=TRUE>>=
yh.ch <- data.frame(ch=apply(y.Mh, 1, paste, collapse=""))
mark.Mh <- mark(data=yh.ch, model="HugHet", silent=TRUE) ## Default model
@
\pause
\vfill
