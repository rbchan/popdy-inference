...
Note: the probabilities must sum to 1 and the counts must sum to $N$. <br>
:::
...
## Simulating removal data, no covariates
...
knitr::kable(y1[1:10,], format="latex", booktabs=TRUE, table.envir="table")
```
...
## Bayesian multinomial *N*-mixture models
...
would be specific to each pass. <br>
...
## Double observer sampling (independent)
...
## Double observer sampling (dependent)
...
## Simulating dependent double observer data
...
##  Dependent double observer model in JAGS
...
## Additional remarks on double observer models
