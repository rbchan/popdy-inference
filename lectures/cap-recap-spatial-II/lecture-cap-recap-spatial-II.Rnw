...
<<lambda-post,size='scriptsize',fig.height=5,fig.show="hide">>=
...
\begin{frame}[fragile]
  \frametitle{Realized density surface}
<<lambda-r,size='scriptsize',fig.height=3.1>>=
lambda.r.mean <- raster(t(apply(lambda.r.post, c(1,2), mean)))
lambda.e.mean <- raster(matrix(lambda.post.mean, sqrt(n.pixels)))
