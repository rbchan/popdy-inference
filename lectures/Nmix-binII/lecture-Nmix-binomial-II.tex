...
\begin{frame}
  ...
  \]
  where `fit' describes how well the model fits the data and
...
{\centering \includegraphics[width=0.5\linewidth]{figure/Elam-pred-map2-1} 
...
%    \sum_{i=1}^M V_s(\log(p(y_i|\theta_s)))
  ...
  \[
%     \mathrm{penalty} = \sum_{i=1}^M \mathrm{Var}(p(y_i|\theta_s)))
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in is.mcmc(x): lazy-load database '/home/rchandler/courses/popdy-inference/lectures/Nmix-binII/cache/bugs-mcmc3\_994dbb2156536b8ea8946744b5281791.rdb' is corrupt}}\end{kframe}
\end{knitrout}
    \end{column}
    \begin{column}{0.5\textwidth}
\begin{knitrout}\scriptsize
...
\hlstd{waic.table}\hlopt{$}\hlstd{deltaWAIC} \hlkwb{<-} \hlstd{waic.table}\hlopt{$}\hlstd{WAIC}\hlopt{-}\hlkwd{min}\hlstd{(waic.table}\hlopt{$}\hlstd{WAIC)}
\hlstd{waic.table} \hlkwb{<-} \hlstd{waic.table[}\hlkwd{order}\hlstd{(waic.table}\hlopt{$}\hlstd{WAIC),]}
\hlstd{knitr}\hlopt{::}\hlkwd{kable}\hlstd{(waic.table,} \hlkwc{format}\hlstd{=}\hlstr{"latex"}\hlstd{,} \hlkwc{booktabs}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
\begin{tabular}{lrr}
\toprule
...
1 & 116.6816 & 1.252070\\
2 & 121.2653 & 5.835729\\
4 & 142.9907 & 27.561159\\
5 & 152.1605 & 36.730935\\
\addlinespace
6 & 152.1605 & 36.730935\\
\bottomrule
\end{tabular}
...
\begin{frame}
  ...
  \pause
  \vfill
  Goodness-of-fit method assess overdispersion and underdispersion. \\
  \pause
  \vfill
...
\begin{frame}[fragile]
  ...
  \pause
  \vfill
  In other words, the $p$-value should be $>0.05$ if the model fits
  the data well.
\end{frame}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fitstat} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{fm}\hlstd{) \{}
    \hlkwd{return}\hlstd{(}\hlkwd{c}\hlstd{(}\hlkwc{SSE}\hlstd{=}\hlkwd{sum}\hlstd{(}\hlkwd{residuals}\hlstd{(fm)}\hlopt{^}\hlnum{2}\hlstd{,} \hlkwc{na.rm}\hlstd{=}\hlnum{TRUE}\hlstd{)))} \hlcom{## Sum of squared-residuals}
\hlstd{\}}
...
## SSE 23.2           2.16             5.61        0.328
## 
## t_B quantiles:
##      0% 2.5% 25% 50% 75% 97.5% 100%
## SSE 6.9   11  17  20  24    33   39
## 
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pcount}\hlstd{(}\hlopt{~}\hlstd{temp} \hlopt{~} \hlstd{elevation.s}\hlopt{+}\hlstd{utmN.s,} \hlkwc{data}\hlstd{=grouse.umf,} \hlkwc{K}\hlstd{=}\hlnum{50}\hlstd{,} \hlkwc{mixture}\hlstd{=}\hlstr{"NB"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning: 5 sites have been discarded because of missing data.}}\begin{verbatim}
## 
## Call:
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pcount}\hlstd{(}\hlopt{~}\hlstd{temp} \hlopt{~} \hlstd{elevation.s}\hlopt{+}\hlstd{utmN.s,} \hlkwc{data}\hlstd{=grouse.umf,} \hlkwc{K}\hlstd{=}\hlnum{50}\hlstd{,} \hlkwc{mixture}\hlstd{=}\hlstr{"ZIP"}\hlstd{)}
\end{alltt}
...
##     y.new[i,j] ~ dbin(p[i,j], N[i])          # Predicted data
##     resid.new[i,j] <- y.new[i,j]-N[i]*p[i,j] # Predicted residual
##   }
## }
## SSE <- sum((y[i,j]-resid[i,j])^2)             # Fit stat
## SSE.new <- sum((y.new[i,j]-resid.new[i,j])^2) # Predicted fit stat
## 
## totalAbundance <- sum(N[1:nSites])
## 
## }
