...
\hldef{dat}\hlopt{$}\hldef{y} \hlkwb{<-} \hlkwd{rnorm}\hldef{(n,} \hlkwc{mean} \hldef{=} \hlopt{-}\hlnum{1} \hlopt{+} \hlnum{2}\hlopt{*}\hldef{dat}\hlopt{$}\hldef{x2,} \hlkwc{sd} \hldef{=} \hlnum{2}\hldef{)}
...
\hldef{fm3} \hlkwb{<-} \hlkwd{glm}\hldef{(y}\hlopt{~}\hldef{x2, gaussian, dat)}  \hlcom{# Correct model}
...
\begin{frame}[fragile]
  \frametitle{Overfitting}
  \centering
  \includegraphics[width=0.8\textwidth]{figure/overfit-1} \\
\end{frame}
