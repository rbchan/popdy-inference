...
\begin{frame}[fragile]
  \frametitle{Spatial prediction}
<<elev-sp,fig.width=9.7,out.width='80%',fig.align='center',size='scriptsize'>>=
...
\begin{frame}[fragile]
  \frametitle{Spatial prediction}
<<lam-pred-map,size='scriptsize',results='hide',fig.width=10.5,out.width='90%',fig.align='center',cache=TRUE>>=
...
\begin{frame}[fragile]
  ...
  \vspace{-6pt}
<<bugs-mcmc,size='footnotesize',message=FALSE,cache=FALSE,results='hide'>>=
library(jagsUI)
...
\vfill
  Fit Model 2
  \vspace{-6pt}
<<bugs-mcmc2,size='footnotesize',message=FALSE,cache=FALSE,results='hide'>>=
...
\begin{frame}[fragile]
  ...
  Model 3
  \vspace{-6pt}
<<bugs-mcmc3,size='tiny',message=FALSE,cache=FALSE,results='hide'>>=
...
@ 
  Model 4
  \vspace{-6pt}
<<bugs-mcmc4,size='tiny',message=FALSE,cache=FALSE,results='hide'>>=
...
@ 
  Model 5
  \vspace{-6pt}
<<bugs-mcmc5,size='tiny',message=FALSE,cache=FALSE,results='hide'>>=
...
@ 
  Model 6
  \vspace{-6pt}
<<bugs-mcmc6,size='tiny',message=FALSE,cache=FALSE,results='hide'>>=
...
waic.table$deltaWAIC <- waic.table$WAIC-min(waic.table$WAIC)
waic.table <- waic.table[order(waic.table$WAIC),]
knitr::kable(waic.table)
@   
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Goodness-of-fit in `unmarked'}
<<parboot,size='footnotesize',warning=FALSE,cache=TRUE,fig.width=9,out.width='70%',fig.align='center'>>=
fitstat <- function(fm) {
    return(c(SSE=sum(residuals(fm)^2))) ## Sum of squared-residuals
}
pb <- parboot(fm1, statistic=fitstat, nsim=200, ncores=3); plot(pb)
@   
\end{frame}
