...
\vfill
  ...
  \vspace{-6pt}
<<ji-M0-aug,size='scriptsize',message=FALSE,report=FALSE,echo=1:2>>=
ji.SCR0 <- function() {
  si <- cbind(runif(M), runif(M))
  for(i in 1:nrow(y)) {
    traps_with_dets_i <- rowSums(y[i,,]>0)
    si[i,] <- colMeans(x[traps_with_dets_i,,drop=FALSE])
  }
  list(z=rep(1,M), psi=runif(1), s=si,
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
    ...
    \item Import the bear data formatted for JAGS (using the command
      ...
      model with occasion-specific $g_0$ in JAGS, and
      compare estimates and 95\% CIs for $g_{0}$, $\sigma$, $E(N)$ and
      $N$ with the results from secr. 
  \end{enumerate}
  \vfill
