...
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{n) \{}
    ...
    \hlkwa{for}\hlstd{(j} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(traps.i)) \{}
        \hlkwd{segments}\hlstd{(s.cap[i,}\hlnum{1}\hlstd{], s.cap[i,}\hlnum{2}\hlstd{],}
                 \hlstd{x[traps.i[j],}\hlnum{1}\hlstd{], x[traps.i[j],}\hlnum{2}\hlstd{],} \hlkwc{col}\hlstd{=}\hlkwd{gray}\hlstd{(}\hlnum{0.6}\hlstd{))}
    \hlstd{\}}
\hlstd{\}}
...
%      rather than a ``global behavioral response'' model in secr.
    \item Import the bear data and compute the capture frequencies and the number of spatial recapture. 
    \item Fit a model in `secr' and JAGS where $g_0$ varies among
      occasions. Compare estimates and 95\% CIs for $g_0$, $\sigma$, $E(N)$ and $N$. 
  \end{enumerate}
  \vfill
